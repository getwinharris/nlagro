<?php
// STANDALONE EMERGENCY DATABASE FIX
...
$sql = "DELETE FROM `" . $db_prefix . "setting` WHERE `code` = 'startup' AND `key` = 'startup_currency'";

echo "<p>Executing query: <code>" . $sql . "</code></p>";

if ($conn->query($sql) === TRUE) {
    if ($conn->affected_rows > 0) {
        echo "<p style=\"font-weight: bold; color: green;\">SUCCESS: The faulty database record was deleted.</p>";
    } else {
        echo "<p style=\"font-weight: bold; color: orange;\">NOTICE: The query ran successfully, but the faulty record was not found. It may have been deleted already.</p>";
    }
